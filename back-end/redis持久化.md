...
以上为redis持久化的主要方式，当然你也可以直接关闭持久化功能，这样数据就只会存在于服务器中了

## 方式对比
了解 RDB持久化 和 AOF持久化的差异是非常重要的，接下来对比一下RDB和AOF两种数据持久化的异同。
...
因为以上提到的种种原因， 未来Redis官方可能会将 AOF 和 RDB 整合成单个持久化模型。 （这是一个长期计划。）
